  ...
  return (
    <div className="rounded-lg border border-border bg-card overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
        ...
        {report.markup ? (
          <span
            title="Spend and every derived cost are inflated by this. Never printed on the client's copy."
            className="shrink-0 rounded-md border border-primary/40 bg-primary/10 px-2 py-1 text-[10px] font-semibold tracking-wide text-primary uppercase"
          >
            +{Math.round(report.markup * 1000) / 10}% markup · internal
          </span>
        ) : null}
        <button
          onClick={() => {

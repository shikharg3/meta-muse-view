...
export const TOOLS: AnthropicTool[] = [
  ...
  },
  {
    name: "list_accounts",
    description:
      "Returns `{ mappingSyncedAt, accounts }`. `accounts` lists EVERY ad account with its Meta status — ACTIVE, PAUSED, or DISABLED (DISABLED = suspended/disabled by Meta, with the disable reason) — the CURRENT client that owns it, that client's Notion board status (e.g. Live, Paused), and `isActiveAccount`: TRUE only when the account is the client's designated Notion 'Active Account ID' (FALSE for an 'Other ad accounts' entry or a manually-added account). When the user asks specifically about the 'Active Account ID' (and NOT 'other'/all accounts), filter to isActiveAccount=true. Each DISABLED account also carries `disabledSince` (YYYY-MM-DD, from Meta's status-change log; null if not captured). `mappingSyncedAt` is when the Notion client→account mapping was last refreshed — if the user says they just edited Notion / 'a few minutes back', WARN that board changes after `mappingSyncedAt` are not reflected yet (they should hit Sync now). Use this for any question about suspended/disabled accounts, when they were disabled, or to cross-reference Notion client status against account suspension (e.g. 'which Live clients have a disabled Active Account ID'). All accounts in one call; never loop.",

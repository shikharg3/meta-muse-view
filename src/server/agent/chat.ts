...
export async function buildSystemPrompt(today = new Date()): Promise<string> {
  ...
  return [
    ...
    "- Some clients are AGENCIES that group several brands under one client name. A brand (e.g. 'Lucky Rebel') is NOT a client name: resolve it with get_client_stats (it fuzzy-matches brands too — the result carries `matchedBrand`/`brandNote`) or search_entities (whose `brands` list maps a brand to its agency client). Never call a brand 'not started' or unknown before checking; when reporting a brand, state it's grouped under the agency client and that figures are client-level (not split per brand).",
